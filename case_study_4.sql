...
-- 1. How many unique nodes are there on Data Bank System?
-- Here each region has some some nodes.
SELECT COUNT(DISTINCT CONCAT(region_id,'-',node_id)) as no_of_unique_nodes
FROM customer_nodes;
...
SELECT customer_id, node_id, start_date, end_date, end_date-start_date,
       lag(node_id) over(partition by customer_id order by start_date) last_node_id,
       CASE
...
SELECT txn_type, COUNT(1) unique_count, SUM(txn_amount) total_amount
FROM customer_transactions
GROUP BY txn_type;

-- 2.What is the average total historical deposit counts and amounts for all customers?
WITH CTE AS (
    SELECT customer_id, SUM(txn_amount) total, COUNT(1) count
    FROM customer_transactions
    WHERE txn_type = 'deposit'
    GROUP BY customer_id
)
SELECT AVG(count) avg_count, AVG(total) avg_amount
FROM CTE;
...
WITH CTE AS (
    SELECT EXTRACT (MONTH FROM txn_date) as month, customer_id,
       ...
       SUM(CASE WHEN txn_type = 'withdrawal' OR txn_type = 'purchase' THEN 1 ELSE 0 END) AS wdrw_prch
    FROM customer_transactions
    GROUP BY month, customer_id
)
SELECT month, SUM(CASE WHEN deposit > 0 AND wdrw_prch > 0 THEN 1 ELSE 0 END) count
FROM CTE
GROUP BY month;
